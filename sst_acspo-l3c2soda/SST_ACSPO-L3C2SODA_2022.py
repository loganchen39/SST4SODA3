'''
Description: Compute binned/average result on SODA 1-degree grid from 0.02-degree ACSPO L3C daily SST.
Author: Ligang Chen
...
DIR_AC = '/glade/scratch/lgchen/data/ACSPO_L3C'
FN_AC_TEST = DIR_AC + "/test/20211201120000-STAR-L3C_GHRSST-SSTsubskin-AVHRRG_N15_N-ACSPO_V2.81-v02.0-fv01.0.nc"
...
sst_soda_5day_avg = np.zeros((360, 180), dtype=np.float32, order='F')  # only nighttime
...
jday_20220103 = datetime.date(2022, 1 , 3 )
# jday_20220110 = datetime.date(2022, 1 , 10)
...
while jday <= jday_20221231:
    ...
    while idy <= jday + datetime.timedelta(days=2):
        ...
        for (sat_dir, sat_fn) in SAT_NM: 
            fn = DIR_AC + '/' + sat_dir + '/' + str_date + '120000-STAR-L3C_GHRSST-SSTsubskin-AVHRRG_' + sat_fn + '_N-ACSPO_V2.81-v02.0-fv01.0.nc'
    ...
    fn_sst_soda_5day = str_jday+'_sst_soda_5day_avg_ACSPO_L3C.nc'
    ds_sst_soda_5day.to_netcdf(DIR_AC+'/l3c2soda/'+fn_sst_soda_5day)

    # write out to text file for SODA use
    fn_sst = DIR_AC + '/l3c2soda/' + '2022_sst_soda_5day_avg_ACSPO_L3C.bin'
